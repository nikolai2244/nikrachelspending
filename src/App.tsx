...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'allocations' | 'transactions' | 'insights'>('overview');
  ...
  const [theme, setTheme] = useState<'carbon' | 'amoled'>(() => {
    return (localStorage.getItem('wealth_app_theme') as 'carbon' | 'amoled') || 'carbon';
  });

  const isRachel = currentUser === 'Rachel' || currentUser === 'Big Mami';
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-100 font-sans select-none antialiased flex flex-col justify-center py-0 md:py-6 px-0 md:px-6">
      ...
      <div className={`w-full max-w-6xl mx-auto ${theme === 'amoled' ? 'bg-[#000000]' : 'bg-zinc-950'} border border-zinc-900 rounded-none md:rounded-[28px] shadow-2xl flex flex-col md:flex-row overflow-hidden relative transition-colors duration-500`} style={{ minHeight: 'calc(100vh - 3rem)', minWidth: '320px' }}>
        ...
        <aside className="hidden md:flex md:w-72 border-r border-zinc-900 bg-black/40 flex-col justify-between p-6 shrink-0 relative z-10">
          <div className="space-y-8">
            ...
            <div className="space-y-1.5">
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all cursor-pointer text-xs font-semibold ${activeTab === 'overview'
              ...
              {!isRachel && (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all cursor-pointer text-xs font-semibold ${activeTab === 'allocations'
                    ? 'bg-[#00e1ff]/10 border border-[#00e1ff]/20 text-[#00e1ff]'
                    : 'text-zinc-500 hover:text-white border border-transparent'
                    }`}
                >
              ...
              {!isRachel && (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all cursor-pointer text-xs font-semibold ${activeTab === 'transactions'
                    ? 'bg-[#00e1ff]/10 border border-[#00e1ff]/20 text-[#00e1ff]'
                    : 'text-zinc-500 hover:text-white border border-transparent'
                    }`}
                >
              ...
              {!isRachel && (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all cursor-pointer text-xs font-semibold ${activeTab === 'insights'
                    ? 'bg-[#00e1ff]/10 border border-[#00e1ff]/20 text-[#00e1ff]'
                    : 'text-zinc-500 hover:text-white border border-transparent'
                    }`}
                >
          ...
          <div className="border-t border-zinc-900 pt-4 flex items-center justify-between">
            ...
            <div className="flex gap-1.5">
              <button
                onClick={() => {
                  const newTheme = theme === 'amoled' ? 'carbon' : 'amoled';
                  setTheme(newTheme);
                  localStorage.setItem('wealth_app_theme', newTheme);
                }}
                className="p-1 px-1.5 rounded-lg bg-zinc-900/60 hover:bg-zinc-900 border border-zinc-850 hover:border-[#00e1ff]/30 text-zinc-500 hover:text-[#00e1ff] transition-colors cursor-pointer"
                title="Toggle Theme"
              >
                {theme === 'amoled' ? <Moon className="w-3.5 h-3.5" /> : <Sun className="w-3.5 h-3.5" />}
              </button>
              <button
                onClick={() => {
                  setCurrentUser(null);
        ...
        <div id="spending-viewport" className={`flex-1 flex flex-col overflow-hidden relative z-10 ${theme === 'amoled' ? 'bg-[#000000]' : 'bg-zinc-950'} transition-colors duration-500`}>
          ...
          <div className="flex-1 overflow-y-auto px-4 md:px-8 py-5 md:py-6 space-y-6 scrollbar-none pb-28 md:pb-8">
            ...
            <AnimatePresence mode="wait">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              className={`px-2.5 py-1 rounded-lg text-[9px] font-semibold tracking-wider uppercase font-mono border transition-all flex items-center gap-1.5 cursor-pointer ${recalibrating
              ...
              {/* TAB 4: ALLOCATIONS & ALL SHEET SYNC CONFIGS */}
              {activeTab === 'allocations' && (
                ...
                >
                  <div className="max-w-xl mx-auto space-y-8">
                    <BudgetCards
                      budgets={budgets}
                      onBudgetChange={handleBudgetLimitChange}
                    />

                    <div className="pt-6 border-t border-zinc-900">
                      <h4 className="text-xs uppercase font-bold tracking-widest text-zinc-400 px-1 mb-4 flex items-center gap-1.5">
                        <FileSpreadsheet className="w-3.5 h-3.5 text-[#00e1ff]" />
          ...
          <nav id="mobile-tabs-dock" className="fixed bottom-5 left-1/2 -translate-x-1/2 w-[90%] bg-zinc-900/95 border border-zinc-800/80 rounded-2xl shadow-2xl backdrop-blur-lg px-4 py-2 flex items-center justify-center gap-8 z-45 md:hidden">
            <button
              onClick={() => setActiveTab('overview')}
              className={`flex flex-col items-center gap-1.5 p-2 transition-all cursor-pointer ${activeTab === 'overview' ? 'text-[#00e1ff]' : 'text-zinc-500 hover:text-white'
                }`}
            >
            ...
            {!isRachel && (
              <button
                onClick={() => setActiveTab('allocations')}
                className={`flex flex-col items-center gap-1.5 p-2 transition-all cursor-pointer ${activeTab === 'allocations' ? 'text-[#00e1ff]' : 'text-zinc-500 hover:text-white'
                  }`}
              >
            ...
            {!isRachel && (
              <button
                onClick={() => setActiveTab('transactions')}
                className={`flex flex-col items-center gap-1.5 p-2 transition-all cursor-pointer ${activeTab === 'transactions' ? 'text-[#00e1ff]' : 'text-zinc-500 hover:text-white'
                  }`}
              >
            ...
            {!isRachel && (
              <button
                onClick={() => setActiveTab('insights')}
                className={`flex flex-col items-center gap-1.5 p-2 transition-all cursor-pointer ${activeTab === 'insights' ? 'text-[#00e1ff]' : 'text-zinc-500 hover:text-white'
                  }`}
              >
