...
export function isRecurringMerchant(merchant: string): boolean {
  const m = String(merchant || '').toLowerCase().trim();
  const recurringTerms = ['netflix', 'spotify', 'hulu', 'pfcu', 'insurance', 'geico', 'rent', 'mortgage', 'utility', 'utilities', 'internet', 'comcast', 'xfinity', 'at&t', 'verizon', 't-mobile', 'apple', 'amazon prime', 'gym', 'planet fitness', 'car payment', 'loan'];
  return recurringTerms.some(term => m.includes(term));
}
...
export function processImportedSheetRows(imported: Transaction[]) {
  ...
  const knownCategories = ['Food', 'Housing', 'Transport', 'Entertainment', 'Shopping', 'Health', 'Utilities', 'Gas', 'Bill', 'Misc', 'Lend', 'Other'];
